...
void StepFrame(Paddle& paddle) {
	...
	int cfx[5] = { 1, 2, 3, 4, 5 };
	for (int i = 0; i < 5; i++) {
		std::string cat = std::to_string(cfx[i]);
		Play::DrawDebugText({ 16, 16 * (i + 2) },cat , Play::cGrey, 0); //added an overload in the playbuffer library, so that it can take std::string and not just char array
	}
}
